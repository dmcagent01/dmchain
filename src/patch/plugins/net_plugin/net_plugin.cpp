...
#include "include/eosio/net_plugin/net_plugin.hpp"
#include "include/eosio/net_plugin/protocol.hpp"
...
namespace eosio {
   ...
   // add a structure to save the records of p2p connections.
   struct p2p_peer_record {
      std::string peer_address;
      fc::time_point_sec expiry;
      bool is_config { true };
   ...
   public:
      ...
      std::map<string, p2p_peer_record> p2p_peer_records;
   ...
   public:
      ...
      void send_p2p_request(const connection_ptr& c);
...
#pragma GCC diagnostic pop
   ...
   public:
      ...
      void send_p2p_response( bool discoverable, const std::string& p2p_peer_list );
      ...
      void handle_message(const request_p2p_message& msg);
      void handle_message(const response_p2p_message& msg);
   ...
   struct msg_handler : public fc::visitor<void> {
      ...
      void operator()(const request_p2p_message& msg) const{
         // continue call to handle_message on connection strand
         peer_dlog( c, "received request_p2p_message" );
         c->handle_message(msg);
      }

      void operator()(const response_p2p_message& msg) const{
         // continue call to handle_message on connection strand
         peer_dlog( c, "received response_p2p_message" );
         c->handle_message(msg);
      }
   ...
   void connection::send_p2p_response( bool discoverable, const std::string& p2p_peer_list ) {
      try {
         enqueue(net_message(response_p2p_message { discoverable, p2p_peer_list }));
      } catch (...) {
   ...
   void connection::handle_message(const request_p2p_message& msg) {
      peer_ilog(this, "whether to enable p2p discovery -- ${result}", ("result", msg.discoverable));
      std::string rec_req_p2p_mes;
      for (const auto& ergodic_record : my_impl->p2p_peer_records) {
         if (ergodic_record.second.discoverable) {
            rec_req_p2p_mes.append(ergodic_record.second.peer_address + "#");
         }
      }
      if (my_impl->p2p_discoverable or rec_req_p2p_mes.size() > 0) {
         send_p2p_response(my_impl->p2p_discoverable, rec_req_p2p_mes);
      }
   }

   void connection::handle_message(const response_p2p_message& msg) {
      peer_ilog(this, "whether to enable p2p discovery -- ${result}, and the length is ${length}", ("result", msg.discoverable)("length", msg.p2p_peer_list.size()));
      ...
      if (!(peer_record->second.is_config and msg.p2p_peer_list.length() > 0)) {
         return;
      }
      int start { 0 };
      std::string qualifier { "#" };
      std::string p2p_peer;
      std::size_t index = msg.p2p_peer_list.find(qualifier, start);
      std::size_t msg_length = msg.p2p_peer_list.size();
      for (;index <= msg_length;) {
         // add a read lock.
         std::shared_lock<std::shared_mutex> read_lock(my_impl->connections_mtx);
         if (my_impl->max_nodes_per_host <= my_impl->connections.size() or my_impl->max_nodes_per_host <= my_impl->p2p_peer_records.size()) {
            read_lock.unlock();
            return;
         }
         read_lock.unlock();
         p2p_peer = msg.p2p_peer_list.substr(start, index - start);
         if (p2p_peer.size() < 3) {
            break;
         }
         start = index + qualifier.size();
         index = msg.p2p_peer_list.find(qualifier, start);
         if (my_impl->find_connection(p2p_peer))
            continue;
         my_impl->supplied_peers.emplace_back(p2p_peer);
         p2p_peer_record p2p_link;
         p2p_link.peer_address = p2p_peer;
         my_impl->p2p_peer_records.emplace(std::pair<std::string, eosio::p2p_peer_record>(p2p_peer, p2p_link));
         my_impl->connect(p2p_peer);
      }
   }
   ...
   void net_plugin_impl::send_p2p_request(const connection_ptr& c) {
      if (!(p2p_discoverable and request_p2p_flag)) {
         return;
      }
      ...
      if (peer_record->second.is_config and !peer_record->second.connected) {
         c->send_p2p_request(p2p_discoverable);
         peer_record->second.connected = true;
      } else {
         bool stop_flag = true;
         for (const auto& record : p2p_peer_records) {
            if (record.second.is_config and !(record.second.connected or record.second.expiry < time_point::now())) {
               stop_flag = false;
               break;
            }
         }
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         // Provide external configuration items to set whether to enable p2p discovery. The default status is not enabled
         ("p2p-discoverable", bpo::value<bool>()->default_value(false), "Allow p2p node discovery, but do not start by default.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         // Assign the value obtained by the configuration item to net_ plugin_ P2p of impl object_ Discoverable property.
   ...
   void net_plugin::plugin_startup() {
      ...
      app().post(priority::highest, [my=my, listen_endpoint](){
         ...
         for (const auto& seed_node : my->supplied_peers) {
            // Connect the external configuration item p2p of the node_peer add address to p2p_peer_records association container.
            eosio::p2p_peer_record p2p_link;
            p2p_link.peer_address = seed_node;
            p2p_link.expiry = time_point_sec((time_point::now()).sec_since_epoch() + 10);
            my->p2p_peer_records.emplace(std::pair<std::string, eosio::p2p_peer_record>(seed_node, p2p_link));
