...
namespace chain {
    ...
    fc::variant fc_value(Isolate* isolate, const JSValue& var)
    {
        ...
        if (var->IsUndefined() || var->IsNull())
            ...
            return fc::variant(var->ToBigInt(isolate->context()).ToLocalChecked()->Int64Value());
        } else if (var->IsNumber() || var->IsNumberObject()) {
            return fc::variant(var->NumberValue(context).ToChecked());
            ...
            obj_ptr<Buffer_base> buf = Buffer_base::getInstance(var);
            if (buf != NULL) {
                exlib::string strBuf;
                fc::blob data;

                buf->toString(strBuf);
                data.data.resize(strBuf.length());
                memcpy(data.data.data(), strBuf.c_str(), strBuf.length());
